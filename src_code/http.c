...
//处理静态请求
void serve_static(int fd, char *filename, size_t filesize, http_out_t *out)
{
		//响应头缓冲区（512字节）和数据缓冲区（8192字节）
		char header[MAXLINE];
		char buff[SHORTLINE];
		struct tm tm;
...
int error_process(struct stat* sbufptr, char *filename, int fd)
{
    // 处理文件找不到错误
		...
		{
				do_error(fd, filename, "404", "Not Found", "fasver can't find the file");
				return 1;
		}
    ...
    // 处理权限错误
		...
		{
				do_error(fd, filename, "403", "Forbidden", "fasver can't read the file");
				return 1;
		}
...
void do_request(void* ptr)
{
		...
		while (1)
		{
        // plast指向缓冲区buff当前可写入的第一个字节位置，这里取余是为了实现循环缓冲
		...
		//修改已经注册描述符的事件类型
		//重置定时器，每等待下一次请求均生效
		epoll_mod(request->epoll_fd, request->fd, request, (EPOLLIN | EPOLLET | EPOLLONESHOT));
		add_timer(request, TIMEOUT_DEFAULT, http_close_conn);

		//每完成一次请求报文的响应都会return以移交出worker线程使用权，并在未断开TCP连接情况下通过epoll监听下一次请求
		return;
